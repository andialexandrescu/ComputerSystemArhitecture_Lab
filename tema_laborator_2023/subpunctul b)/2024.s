.data
    ...
    matrixIndex: .space 4
    key: .space 1600;# 1 * 80, un array de maxim 80 de elem de tip byte
    keyIndex: .space 4
...
et_comparare_lungimi:
    ...
    pusha
    pushl maxMessageLength
    pushl $formatmaxMessageLength
    call printf
    addl $8, %esp
    popa
...
init_for_caracter_mesaj:
    lea message, %edi
    movl $0, mIndex
for_caracter_mesaj: ;# for(vIndex=0; vIndex<messageLength; vIndex++)
    movl mIndex, %ecx
    cmp %ecx, messageLength
    je init_afis_binCh
